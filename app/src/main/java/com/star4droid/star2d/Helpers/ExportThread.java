...
public class ExportThread extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            FileUtil.copyFile(export_temp + "/manifest", export_temp + "/AndroidManifest.xml"); // Fallback
    ...
    public void onSuccess(final String message) {
        post(() -> {
            ...
            alertD.setNegativeButton("Cancel", (dl, which) -> {

            });
            alertD.setPositiveButton("Install", (dl, which) -> {
                android.os.StrictMode.setVmPolicy(new android.os.StrictMode.VmPolicy.Builder().build());
                if (android.os.Build.VERSION.SDK_INT >= 24) {
                    try {
                        java.lang.reflect.Method m = android.os.StrictMode.class.getMethod(
