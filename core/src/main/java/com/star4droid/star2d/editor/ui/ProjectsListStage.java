...
public class ProjectsListStage extends Stage {
    ...
    private Drawable drawable(String name) {
        ...
        try {
            ...
            // Try loading from internal files, fallback to logo if missing
            try {
                FileHandle file = Gdx.files.internal("images/" + name);
                if (!file.exists()) {
                    file = Gdx.files.internal("images/" + name + ".png");
                }
                if (!file.exists()) {
                    file = Gdx.files.internal("images/logo.png");
                }
                return Utils.getDrawable(file);
            } catch (Exception | Error e2) {
                return VisUI.getSkin().getDrawable("default");
            }
        }
    }
